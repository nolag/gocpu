...
func assertBoundary(t *testing.T, mem memory.Memory, startIndex uint64, notAllowedAmount uint64, fixByOffset bool, message string) {
	// Given
	expectedR := memory.AccessViolationError{Location: startIndex, NumBytes: notAllowedAmount, WasRead: true}
	expectedW := memory.AccessViolationError{Location: startIndex, NumBytes: notAllowedAmount, WasRead: false}
